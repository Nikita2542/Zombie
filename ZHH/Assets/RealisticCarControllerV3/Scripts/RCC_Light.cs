...
public class RCC_Light : RCC_Core {
    ...
    private void Update() {
        ...
        //  If projector option in RCC Settings is set to true, use them.
        if (projector)
            Projectors();
    ...
    private void Projectors() {
        ...
        projector.material.color = LightSource.color * (LightSource.intensity / 5f);

        //  Setting range and field of view of the projector.
        projector.farClipPlane = Mathf.Lerp(10f, 40f, (LightSource.range - 50) / 150f);
        projector.fieldOfView = Mathf.Lerp(40f, 30f, (LightSource.range - 50) / 150f);
